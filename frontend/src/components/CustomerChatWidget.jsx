    ...
    return (
        <div className="fixed bottom-6 right-6 z-50">
                ...
                <div className="bg-white w-80 sm:w-96 rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden mb-4 transition-all duration-300 ease-in-out">
                        ...
                        {messages.map(m => (
                            <div key={m.id} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`flex items-end gap-2 max-w-[85%] ${m.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${m.sender === 'user' ? 'bg-indigo-100 text-indigo-600' : 'bg-blue-600 text-white'}`}>
                                        {m.sender === 'user' ? <User size={14} /> : <Bot size={14} />}
                                    </div>
                                    <div className={`p-3 rounded-2xl text-sm ${m.sender === 'user' ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-white border shadow-sm text-gray-800 rounded-bl-sm'} whitespace-pre-wrap leading-relaxed`}>
                                        {m.text}
                                    </div>
                                </div>
                            </div>
                        ...
                        {loading && (
                            <div className="flex justify-start">
                                <div className="bg-white border shadow-sm p-3 rounded-2xl rounded-bl-sm flex gap-1 items-center">
                                    <Loader2 size={16} className="animate-spin text-blue-600" /> <span className="text-xs text-gray-500">Typing...</span>
                                </div>
                            </div>
                        )}
                    ...
                    <form onSubmit={handleSend} className="p-3 border-t bg-white flex items-center gap-2">
                        <input
                            ...
                            className="flex-1 px-4 py-2 bg-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition"
                            disabled={loading}
                        />
                        <button type="submit" disabled={!input.trim() || loading} className="p-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50 transition">
                            <Send size={18} />
                        </button>
                    </form>
                </div>
            ...
            <button
                ...
                className={`p-4 rounded-full shadow-2xl text-white transition-all duration-300 transform hover:scale-105 flex items-center justify-center ${isOpen ? 'bg-gray-800 rotate-90' : 'bg-gradient-to-r from-blue-600 to-indigo-600'}`}
